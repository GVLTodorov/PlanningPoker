// Browser-driven demo recorder: the same flow as PlanningPoker.Tests.Play, scaled up to 12
// simulated players (1 host + 11 guests) to show the board holding a full-size room instead of a
// small one. Reuses DemoPlayer from PlanningPoker.Tests.Play rather than duplicating it. Only the
// host's session is recorded to video. Every wait targets a specific element (never NetworkIdle --
// SignalR's persistent connection means that would never settle), so a stuck flow surfaces as a
// Playwright TimeoutException and a non-zero exit code: this doubles as a live smoke test of a
// larger room's join/pick/reveal/reset, not just a video-generation script.
//
//   dotnet run --project PlanningPoker.Tests.Play.Twelve -c Release -- http://localhost:6232 artifacts/demo-video-twelve
...
string[] playerNames = ["Maya", "Noah", "Priya", "Sam", "Alex", "Liam", "Zoe", "Ethan", "Ivy", "Omar", "Nora", "Kai"];
...
// keeping the other 11 sessions unrecorded keeps the artifact small and focused.
...
// card appearing individually rather than all eleven popping in at once.
...
foreach (var player in players)
{
    ...
    await Task.Delay(rng.Next(250, 600));
}
