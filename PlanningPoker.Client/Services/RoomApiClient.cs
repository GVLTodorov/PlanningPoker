...
public sealed class RoomApiClient
{
    ...
    public async Task<IReadOnlyList<DeckDto>> GetDecksAsync(CancellationToken cancellationToken = default) =>
        await _httpClient.GetFromJsonAsync<List<DeckDto>>("/api/decks", JsonOptions, cancellationToken) ?? [];
    ...
    public async Task<RoomSummaryDto?> CreateRoomAsync(string name, DeckType deckType, CancellationToken cancellationToken = default)
    {
        var response = await _httpClient.PostAsJsonAsync(
            "/api/rooms", new CreateRoomRequest(name, deckType), JsonOptions, cancellationToken);

        return response.IsSuccessStatusCode
            ? await response.Content.ReadFromJsonAsync<RoomSummaryDto>(JsonOptions, cancellationToken)
            : null;
    }

    public async Task<RoomSummaryDto?> GetRoomAsync(string roomId, CancellationToken cancellationToken = default)
    {
